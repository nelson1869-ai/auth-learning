...
| **9** | `helmet()` sa `app.ts`, `middleware/rateLimiter.ts`, `frontend/public/_headers` (CSP), `lib/` (logger, clientIp) + `middleware/requestLogger.ts`, `middleware/errorHandler.ts` | Hardening bago buksan sa lahat: security headers (Day 39), sentral na error handling (Day 41), logs na may request ID (Day 42), limit sa panghuhula ng password (Day 43) |
